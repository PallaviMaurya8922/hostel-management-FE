                ...
                <>
                  <div className="divide-y divide-surface-100 lg:hidden">
                    {visibleRequests.map(request => {
                      const status = getUnifiedRequestStatusDisplay(request);
                      const lockMessage = getModerationLockMessage(request);

                      return (
                        <button
                          key={`${request.requestType}-${getRequestId(request)}-mobile`}
                          type="button"
                          onClick={() => setSelectedRequest(request)}
                          className="w-full px-4 py-4 text-left transition-colors hover:bg-surface-50/80"
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span className="text-xs text-slate-500">{formatRelativeTime(request.created_at)}</span>
                              </div>
                              <p className="mt-2 text-sm font-semibold text-slate-900">{getStudentName(request)}</p>
                              <p className="mt-1 text-xs text-slate-500">{getStudentSubline(request)}</p>
                              <p className="mt-3 text-sm text-slate-700">{getRequestPreviewText(request)}</p>
                              {lockMessage ? (
...
function getRequestPreviewText(request: UnifiedRequest): string {
  if (isLeaveRequest(request)) {
    return `${formatDateRange(request.start_date, request.end_date)} · ${request.reason}`;
  }
  if (isGuestRequest(request)) {
    return `${request.guest_name} · ${request.visit_type === 'overnight' ? 'Overnight visit' : 'Normal visit'} · ${request.purpose}`;
  }
  if (isMaintenanceRequest(request)) {
    return `${request.issue_type} · ${request.description}`;
  }
